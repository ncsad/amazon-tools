...
def check_and_download_scripts():
    """首次运行时下载缺失的脚本文件"""
    missing = [f for f in SCRIPTS.values()
               if not os.path.exists(os.path.join(BASE_DIR, f))]
    if not missing:
        return True, []
    return False, missing

def download_missing(files, progress_cb=None):
    """下载缺失文件，progress_cb(percent, msg)"""
    total = len(files)
    for i, filename in enumerate(files):
        if progress_cb:
            progress_cb(int(i / total * 90), f"下载 {filename}…")
...
def github_raw_url(filename):
    ...
    return (f"https://raw.githubusercontent.com/"
            f"{GITHUB_USER}/{GITHUB_REPO}/main/{encoded}")
...
class FirstRunDialog(QDialog):
    ...
    def _build(self):
        ...
        title = QLabel("🚀  首次运行，正在初始化…")
        title.setStyleSheet("font-size:14px; font-weight:700;")
        layout.addWidget(title)

        desc = QLabel("正在从服务器下载工具文件，请稍候。")
        desc.setStyleSheet("font-size:11px; color:#555555;")
    ...
    def _start(self):
        self.thread = Downloader(self.missing)
    ...
    def _on_done(self, success, msg):
        if success:
            self.status_lbl.setText("✅ 下载完成，可以开始使用了！")
            self.ok_btn.setEnabled(True)
        else:
            self.status_lbl.setText(f"❌ 下载失败：{msg}  请检查网络后重启。")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._build()
        # 首次运行检查
        ok, missing = check_and_download_scripts()
        if not ok:
            dlg = FirstRunDialog(missing, self)
